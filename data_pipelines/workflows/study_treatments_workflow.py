...
def create_study_treats_table() -> pd.DataFrame:
    ...
    for studies_data_pickle_file in os.listdir(directory):
        ...
        with open(studies_file, 'rb') as f:
            ...
            study_treatment_dfs.append(studies_table_df)
...
def parse_study_treatments(studies: typing.List[dict]) -> pd.DataFrame:
    ...
    for study in studies:
        ...
        try: 
            study_treats['interventions'].append([x.get('InterventionMeshTerm', 'NA') for x in study['Study']['DerivedSection']['InterventionBrowseModule']['InterventionMeshList']['InterventionMesh']])
        except KeyError as e:
            study_treats['interventions'].append([])
...
def get_study_id(study_treats: pd.DataFrame) -> pd.DataFrame:
    ...
    merged_table = table.merge(study_ids, left_on="study_id", right_on="nct_id")\
...
def fill_in_treatments(study_treats: pd.DataFrame) -> pd.DataFrame:
    db = create_engine(DATABASE_URL)
    treatments = pd.read_sql("select id as treat_id, name from treatments", db.connect())
    merged_table = study_treats.merge(treatments, how='left', left_on="treatment", right_on="name")\
        .drop(columns=['name'], axis=1)\
        .rename(columns={ 
            'treatment': 'treat_name'
            'treat_id': 'treatment' })

    last_id = treatments['id'].max()
    print("last treatment id " + last_id)

    nan_treats = merged_table[merged_table['treat_id'].isna()]

    new_treatments = nan_treats['treat_name'].to_frame()
    new_treatments['id'] = range(last_id + 1, last_id + 1 + len(new_treatments))
    new_treatments.rename(columns={'treat_name': 'name'})
    ...
    treatments = pd.read_sql("select id as treat_id, name from treatments", db.connect())
    merged_table = study_treats.merge(treatments, how='left', left_on="treatment", right_on="name")\
        .drop(columns=['name', 'treatment'], axis=1)\
        .rename(columns={ 
            'treat_id': 'treatment' })
...
def study_treatments_workflow():
    ...
    study_treats_table = fill_in_treatments(study_treats_table)

    print("adding in study id...")
    study_treats_table = add_study_id(study_treats_table)
