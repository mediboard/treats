...
def create_measurements_table():
    ...
    for studies_data_pickle_file in os.listdir(directory):
        ...
        with open(studies_file, 'rb') as f:
            ...
            measurements_table_df = create_measurements_table_helper(studies=studies_data)
...
def clean_measures_table(measures_table: pd.DataFrame) -> pd.DataFrame:
    ...
    db_measures_table['dispersion'] = db_measures_table['dispersion'].apply(
        lambda x: x if '.' not in x else x[:x.index('.')] + x[x.index('%'):]).apply(lambda x: dispersion_map[x])
